...
- `tolerance`: convergence when the largest squared displacement of any
  ...
  objective flattening out. For `GaussianKernel` on full data, convergence
  never fires before the second iteration, and also triggers when the
  relative objective decrease falls below an internal `rtol = 1e-8` (not
  exposed here); in stochastic mode the Gaussian kernel uses the MM sweep
  and the displacement rule only.
