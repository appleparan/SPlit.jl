...
This document provides context for coding agents working on the SPlit.jl project.
...
- `energydistance(X, Y)`: energy distance between two samples — exact by
  default; pass `subsample` (with `repeats`, `rng`) for a subsampled
  estimate, which carries a positive bias of order `1/subsample`.
- `splitquality(data, result)`: energy distance between the train and test
  rows of a `SplitResult`; lower is better. Exact up to `exact_threshold`
  rows (default 4,000), subsampled estimate above it.
...
2. Apply the kernel's closed-form majorization-minimization update, which
   ...
   2018).
3. When `kappa` is given and smaller than the number of data rows, use the
   stochastic variant of Joseph & Vakayil (2021): resample `kappa` rows per
   iteration and stabilize the update with running averages.
4. Converge when the largest squared displacement of any support point in
   one iteration falls below `tolerance`, or stop at `max_iterations`. In
   stochastic mode the running-average weight decays, so convergence partly
   reflects step-size decay; `SplitResult.converged` reports which happened.
...
- The MM sweep in `optimizer.jl` is the hot loop: keep it allocation-free
  (the coordinate loops exist for that reason) and thread-chunked by
  `n_threads`.
- Large `n` is served by `kappa` (stochastic mode), not by relaxing the
  full-data algorithm.
